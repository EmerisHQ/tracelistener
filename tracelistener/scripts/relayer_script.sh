...
do
    ...
    echo "------- add chains-------------"
    rly chains add -f test/$CHAINID-$a.json --home $RELAYER_HOME

    echo "---------restore keys with existing seeds--------"
    if [ $a == 1 ]
    then
        rly keys restore $CHAINID-$a "$RLYKEY-$a" "$SEED1" --home $RELAYER_HOME

        export SRC=$CHAINID-$a
    else
        rly keys restore $CHAINID-$a "$RLYKEY-$a" "$SEED2" --home $RELAYER_HOME

        export DST=$CHAINID-$a
    fi

    echo "----------create a light client----------"
    rly light init $CHAINID-$a -f --home $RELAYER_HOME
...
rly pth add $SRC $DST $PTH -f test/$PTH.json --home $RELAYER_HOME

echo "-----show keys----------"
rly keys show $SRC --home $RELAYER_HOME
rly keys show $DST --home $RELAYER_HOME

echo "--------link path--------------"
rly tx link $PTH --home $RELAYER_HOME

sleep 2s

echo "-------start relayer--------"
rly start $PTH --home $RELAYER_HOME </dev/null &>/dev/null &

sleep 2s

echo "----- run transfer tx from $SRC to $DST---------"
rly tx transfer $SRC $DST 1000${DENOM} $(rly ch addr $DST) --home $RELAYER_HOME

echo

echo "----- run transfer tx from $DST to $SRC---------"
rly tx transfer $DST $SRC 1000${DENOM} $(rly ch addr $SRC) --home $RELAYER_HOME
